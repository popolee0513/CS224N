...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    loss = np.exp(np.dot(outsideVectors[outsideWordIdx],centerWordVec))/ np.exp(np.dot(outsideVectors,centerWordVec)).sum()
    loss = -np.log(loss)
    gradCenterVec = -outsideVectors[outsideWordIdx]
    prob_dist  = softmax(np.dot(outsideVectors,centerWordVec))
    gradCenterVec += np.dot(outsideVectors.T,prob_dist)
    # w = o, the true 'outside' word vector
    # (P(W|C)-1)*Vc
    # w != o,for all other words.
    # P(W|C)*Vc
    prob_dist[outsideWordIdx]-=1
    gradOutsideVecs = np.dot(prob_dist.reshape(len(prob_dist),1),centerWordVec.reshape(1,len(centerWordVec)))
...
def negSamplingLossAndGradient(
    ...
    loss_1 = -np.log(sigmoid(np.dot(outsideVectors[indices[0]],centerWordVec)))
    loss_2 = 0
    for index in indices[1:]:
        loss_2 -= np.log(sigmoid(-np.dot(outsideVectors[index],centerWordVec)))
    loss = loss_1 + loss_2
    gradCenterVec_1 = (sigmoid(np.dot(outsideVectors[indices[0]],centerWordVec))-1)*outsideVectors[indices[0]]
    gradCenterVec_2 =0
    for index in indices[1:]:
        gradCenterVec_2 += sigmoid(np.dot(outsideVectors[index],centerWordVec))*outsideVectors[index]
    gradCenterVec = gradCenterVec_1 +gradCenterVec_2
    
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for index in indices[1:]:
        gradOutsideVecs[index]+=sigmoid(np.dot(outsideVectors[index],centerWordVec))*centerWordVec
       
    gradOutsideVecs[indices[0]] = (sigmoid(np.dot(outsideVectors[indices[0]],centerWordVec))-1)*centerWordVec
    return loss, gradCenterVec, gradOutsideVecs
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for i in range(len(outsideWords)):
        cur_outside = outsideWords[i]
        outsideWordIdx = word2Ind[cur_outside]
        centerWordIdx = word2Ind[currentCenterWord]
        cur_center = centerWordVectors[centerWordIdx]
        loss_, gradCenter,gradOutside=word2vecLossAndGradient(cur_center,outsideWordIdx,outsideVectors,dataset)
        loss+=loss_
        gradCenterVecs[centerWordIdx]+=gradCenter
        gradOutsideVecs+=gradOutside
