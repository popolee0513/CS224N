...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        #####   和nn.LSTM直接得到編碼完整個序列後的h和output不同，nn.LSTMCell完成的是序列中單個單詞的計算
    ...
    def forward(self, source, target):
        ...
        # 要決定資料要放在cpu 還是 GPU
        #### to_input_tensor  ---> token to indices and padding to same length
        ...
        ###        combined output tensor  (tgt_len, b,  h) ---->self.target_vocab_projection ------> (tgt_len, b, V)
        ...
        target_gold_words_log_prob = torch.gather(P, index=target_padded[1:].unsqueeze(-1), dim=-1).squeeze(-1) * target_masks[1:] ### 忽略padding 的地方
    ...
    def encode(self, source_padded, source_lengths):
        ...
        ###   在 NLP 中，文本數據大都是變長的，為了能夠做 batch 的訓練，需要 padding 到相同的長度，並在實際訓練中忽略 padding 部分的影響。
        ###　　keras : Masking 層   pytorch: torch.nn.utils.rnn.pack_padded_sequence
        x =  self.model_embeddings.source(source_padded)
        packed_seq =  pack_padded_sequence(input=x,lengths=source_lengths, batch_first=False)
        output, (hn, cn) = self.encoder(packed_seq)
        output, _  = pad_packed_sequence(sequence=output)
        enc_hiddens  = torch.permute(output, (1, 0, 2))
        
        ### 做projection 的原因是因為decoder 的hidden state 維度為h 2h--->h 
        h = torch.cat([hn[0],hn[1]], dim=1)
        init_decoder_hidden = self.h_projection(h)
        c = torch.cat([cn[0],cn[1]], dim=1)
    ...
    def decode(self, enc_hiddens, enc_masks,
        ...
        #### 把enc_hiddens mapping 到與decoder相同的空間後在做dot product
        
        y_embed = self.model_embeddings.target(target_padded)
        for y in torch.split(y_embed,1):
            cur_timestep = torch.squeeze(y, 0)
            Ybar_t = torch.cat((cur_timestep,o_prev), dim=1)
    ...
    def step(self, Ybar_t,
        ...
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj,dec_hidden),2)  #(b, src_len, h)*(b,h,1) >>> (b,src_len)
        ### END YOUR CODE
        ...
        alpha_t = torch.unsqueeze(alpha_t,1)  #(b,1,src_len)*(b, src_len, 2h)
        dec_hidden = torch.squeeze(dec_hidden, 2)  #(b, h)
        U_t  = torch.cat((torch.squeeze(torch.bmm(alpha_t,enc_hiddens),1),dec_hidden),1)
