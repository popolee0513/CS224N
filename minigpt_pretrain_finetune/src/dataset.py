...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x = self.data[idx]
        x_length = torch.torch.distributions.Uniform(low=4, high=int(self.block_size*7/8)).sample((1,1)).item()
        x = x[:int(x_length)]
        len_x = len(x)
        mask_proportion = torch.torch.distributions.Uniform(low=0.2, high=0.3).sample((1,1)).item()
        mask_length = int(len_x*mask_proportion)
        mask_content  =x[len(x)//2:len(x)//2+mask_length]
        part_a = x[:len(x)//2]
        part_b = x[len(x)//2+mask_length:]
        x = part_a + self.MASK_CHAR + part_b + self.MASK_CHAR + mask_content + self.MASK_CHAR
        x = x + self.PAD_CHAR*(self.block_size - len(x))
        y = x[1:]
        x = x[:-1]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
